...
- Maintainer asked for a short demo, ASCII was explicitly fine. Built `docs/demo.sh`, which runs turnbreak end to end (install, `mode folder`, a turn that fires, `watch` showing the item, the turn ending) against a scratch `HOME` with `TURNBREAK_NO_BROWSER`/`TURNBREAK_NO_NOTIFY` set, verified to actually work. Tried to record it as an animated SVG with `termtosvg`: it and every other pty-based recorder need a real controlling terminal, and the sandbox this session runs in has none (`sys.stdin.isatty()` is `False`, confirmed with a trivial `echo` test that also recorded blank). So the README embeds real captured output from running `docs/demo.sh`, as a static transcript, rather than an animation. An actual animated recording is one command away on a real terminal: `termtosvg docs/assets/demo.svg -c "bash docs/demo.sh" -g 90x24`.
...
- [ ] Push and verify the README renders correctly, especially the demo. If the maintainer wants an animated recording instead of the current static transcript, run `docs/demo.sh` through `termtosvg` from a real terminal and embed the result.
- [ ] Publish the release.
