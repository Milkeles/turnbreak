...
While reviewing the pushed repo, the maintainer found and fixed real gaps, and asked for others to be fixed here:

- Issue templates were Markdown with front matter. Converted to GitHub's YAML issue forms (`.github/ISSUE_TEMPLATE/bug_report.yml`, `feature_request.yml`, `config.yml`), based on templates the maintainer already uses on another repo (`alekszlat/Training-Dynamic-Gesture-Model`), adapted to keep turnbreak-specific fields (agent, agent version, OS) the source templates didn't have. `config.yml`'s links point at `Milkeles/turnbreak`, not the source repo. Deliberately did not carry over that repo's `.github/SECURITY.md` pattern of listing a co-maintainer's email as a fallback contact. Alex isn't a contributor here, so no such line exists in this repo's `SECURITY.md`.
- `.pre-commit-config.yaml` existed but ran in neither CI nor was mentioned in the README, so it was silent unless a contributor happened to run `pre-commit install` themselves. CI now has its own `lint` job running `uv run pre-commit run --all-files` (three jobs total: `lint`, `typecheck`, `test`, matching the structure of the maintainer's other repo's workflow). Added a `pre-commit` line to `pyproject.toml`'s `dev` extra so `uv run pre-commit` resolves in CI without a separate install step, and re-locked. README gained a `## Development` section covering `uv run pre-commit install` and `uv run pre-commit run --all-files`.
- Added `.github/dependabot.yml`: `uv` ecosystem (native support since March 2025) for `pyproject.toml`/`uv.lock`, plus `github-actions` for the workflow file. Both weekly.
- `README.md`'s install command, `pip install "turnbreak[sources]"`, fails on current Debian and Ubuntu with `externally-managed-environment` (PEP 668), confirmed by the maintainer running it on their own machine. Changed the primary instruction to `pipx install "turnbreak[sources]"`, which is the right tool for installing a CLI globally without touching the OS's Python packages, with a venv fallback for anyone who doesn't want `pipx`.
- Maintainer noted private vulnerability reporting likely needs a public repo. Confirmed: GitHub's own docs describe it as a public-repository feature, and enabling it was not possible on the private repo. Left `SECURITY.md` and `config.yml`'s link as they are, since both are already correct for when the repo goes public; nothing to fix until then.
- Maintainer noted the social preview upload option wasn't showing in Settings. Confirmed: GitHub only allows uploading a repository's first social preview image while the repo is public. A private repo can keep an image it already had before going private, but can't add one for the first time. Nothing to do until the repo is public; still tracked below.
- Next round of maintainer review: `pipx install "turnbreak[sources]"` still failed, this time with "no versions found", because turnbreak was never published to PyPI and never planned to be (checked: no PyPI-publish task exists anywhere in this file; that's separate from the P0 name-availability check). Fixed by installing straight from this repo: `pipx install "turnbreak @ git+ssh://git@github.com/Milkeles/turnbreak.git"`, verified with both a scratch venv and a real `pipx install`. Works today against the private repo using the installer's own SSH key; README's "Alternative installation" section notes the `git+https://` form to switch to once the repo is public.
- Maintainer asked for folder mode, not curated mode, to be the README's primary flow, and for the README to get shorter. Restructured: Install now walks straight to `mode folder`, curated mode (interests, finders, `onboard`) moved to its own `### Curated mode` subsection, and the PEP 668 / no-`pipx` / venv fallback moved into a collapsible `## Alternative installation` section instead of sitting inline.
- Added a CI badge and an MIT license badge near the top of the README, per the maintainer's request to make it "pretty with shield icons."
- Maintainer asked for a short demo, ASCII was explicitly fine. Built `docs/demo.sh`, which runs turnbreak end to end (install, `mode folder`, a turn that fires, `watch` showing the item, the turn ending) against a scratch `HOME` with `TURNBREAK_NO_BROWSER`/`TURNBREAK_NO_NOTIFY` set, verified to actually work. Tried to record it as an animated SVG with `termtosvg` from this session's sandbox: no controlling terminal there (`sys.stdin.isatty()` is `False`), so every pty-based recorder captured blank. The maintainer ran `termtosvg docs/assets/demo.svg -c "bash docs/demo.sh" -g 90x24` on their own machine instead, which worked, and the README now embeds that animated `docs/assets/demo.svg` above the fold.
- Maintainer reviewed that first recording and found it only showed the terminal fallback (`turnbreak watch --once`'s text dump), not the browser-tab product the README claims. Considered a real-browser screen recording instead (built `docs/demo-browser.sh` for it), but the maintainer preferred keeping it text-only and just tightening it up. Condensed `demo.sh`'s output to a spinner plus a bordered excerpt card instead of the full article body, added an explicit `note` line ("a reading tab opens in the browser:") so the ASCII recording says what the real product does even though it can't show it, and deleted `docs/demo-browser.sh`. The maintainer re-recorded with the same `termtosvg` command against the condensed script; the README's embedded SVG now reflects that.
- Maintainer made the repository public themselves. Followed up: swapped the README's remaining `git+ssh://` install URLs to `git+https://` (a public repo needs no SSH key to install from), deleted the now-redundant "once this repo is public" fallback section, added a live link to `SECURITY.md`'s private-vulnerability-reporting instructions (verified live via `gh api repos/Milkeles/turnbreak/private-vulnerability-reporting` returning `{"enabled":true}`), and set the 8 GitHub topics from `docs/positioning.md` via `gh repo edit --add-topic` (the About field was already correct). Checked for a REST API to upload the social preview image while at it: none exists (`gh api -X PUT repos/.../social-preview` returns 404), so that upload stays a manual step in the repo's Settings UI.
- Maintainer asked to add a `.github/FUNDING.yml`, matching the one on `Milkeles/software-doc-templates` (`ko_fi: milkeles`).
- Maintainer asked to remove `.claude/settings.json` (a local Claude Code permission allowlist, not meant to ship) and every `Co-Authored-By: Claude` trailer, since those trailers put Claude on GitHub's contributor list. Both had already been pushed, so this meant a second history rewrite with `git-filter-repo` (the first was the pre-push author-email cleanup two items up) followed by a force-push of `main` and the `v0.1.0` tag. Confirmed with the maintainer first, since this is the one destructive step in this file that touches history already on GitHub: anyone who'd already cloned or forked would see a diverged history after. Maintainer chose to do it anyway. `.claude/` added to `.gitignore` so the file doesn't come back. Verified after: 0 co-author trailers left in `git log --all`, `.claude/settings.json` 404s on the GitHub API, GitHub's contributors list shows only the maintainer, and the full test suite still passes post-rewrite.
- Maintainer uploaded the social preview image themselves once the repo was public (the manual step noted above). Reported seeing the README's embedded demo still looking like the old, uncondensed recording; checked by diffing `raw.githubusercontent.com`'s copy of `docs/assets/demo.svg` against the local file (identical, `source-age: 0`) — the file on GitHub was already correct, so this was a stale browser cache on the maintainer's end, not a repo issue.
- Fixed the topic list's gap noted below: `docs/positioning.md` now lists `copilot-cli` as a ninth topic, and it's live on the repo alongside the original eight.

- [x] Confirm with the maintainer that the repository should go public now.
- [x] Create the GitHub repository, set the About field and topics from `docs/positioning.md` (nine, including `copilot-cli`). Social preview image is uploaded — maintainer did it manually once the repo was public.
- [x] Push and verify the README renders correctly, especially the demo (`docs/assets/demo.svg`).
- [ ] Publish the release.
