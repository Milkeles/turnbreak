...
- [ ] Squash or clean the history if it exposes anything the maintainer would not want public. Blocked on a maintainer decision: the real finding from the item above is 28 commits carrying the real address `milkeles.apps@gmail.com` as the git author email instead of the `hristo@example.com` placeholder later commits use. Nothing else in the history needs cleaning. Once the maintainer says how to handle it, the mechanical fix is a single rewrite of every commit's author identity to the placeholder (for example `git filter-repo --mailmap`, or `git rebase -i --root` with `--committer-date-is-author-date` and an exec step per commit), which changes every commit hash in the repository. Not attempted without explicit instruction, since it is a destructive, hash-changing rewrite of the whole history.
- [x] Tag v0.1.0 locally and write release notes. Release notes in `CHANGELOG.md`'s new `[0.1.0]` section. Tagged locally with `git tag -a v0.1.0`, not pushed, per the P11 boundary.
- [x] Hand the project to the maintainer for testing, and stop. Two open items need the maintainer directly and cannot be closed autonomously: notifications on macOS/Windows (this environment is Linux only), and a fresh install by someone who has never seen the project. One decision is waiting on them: whether to rewrite the 28 commits carrying the real email address (see the history item above) before anything is ever pushed.
